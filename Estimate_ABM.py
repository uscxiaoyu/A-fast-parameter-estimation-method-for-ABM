...
class Diffuse:  # 默认网络结构为节点数量为10000，边为60000的单向随机网络
    ...
    def decision(self, i):  # 个体状态决策规则
        dose = sum([1 for k in self.g.predecessors(i) if self.g.node[k]['state']])
...
if __name__ == '__main__':
    ...
    res = est_abm.solution_search(p0, q0)
    print('    Time elasped: %.2f s' % (time.clock() - t1))
    print('    R2:%.4f    steps:%d' % (res['fitness'], res['steps']))
